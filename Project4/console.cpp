...
public:
    socks4_request(tcp::endpoint& endpoint_){
        vcd2[0] = 4;
        vcd2[1] = 1;
        vcd2[2] = endpoint_.port()/256;
        vcd2[3] = endpoint_.port()%256;
        socks4_destip = endpoint_.address().to_v4().to_bytes();
        userID = "";
        nul = 0;
    }

    boost::array<mutable_buffer, 7> buf(){
        boost::array<mutable_buffer, 7> res = {
            buffer(vcd2, 1), buffer(vcd2+1, 1), buffer(vcd2+2, 1), buffer(vcd2+3, 1),
            buffer(socks4_destip),
            buffer(userID),
            buffer(&nul, 1)
        };
        return res;
...
private:
    unsigned char vcd2[4];
    address_v4::bytes_type socks4_destip;
    string userID;
    unsigned char nul;
};
...
class socks4_reply{
public:
    boost::array<mutable_buffer, 5> buf(){
        boost::array<mutable_buffer, 5> res = {
            buffer(vcd2, 1), buffer(vcd2+1, 1), buffer(vcd2+2, 1), buffer(vcd2+3, 1),
            buffer(socks4_destip)
        };
        return res;
...
private:
    // VN, CD, destination_port[2], destination_ip
    unsigned char vcd2[4];
    address_v4::bytes_type socks4_destip;
};
...
public:
    ...
    void start(){
        ...
        if(set_sock){
            socket_.connect(sock_endpoint);
            socks4_request tmp_request(endpoint_);
            socks4_reply tmp_reply;
            write(socket_, tmp_request.buf(), transfer_all());
            read(socket_, tmp_reply.buf(), transfer_all());
            do_read();
        }
        else{
...
int main(){
    ...
    if(vec_str_find(query_strings, "sh") != -1 && vec_str_find(query_strings, "sp") != -1){
        tcp::resolver::query query(query_strings[vec_str_find(query_strings, "sh") + 1], query_strings[vec_str_find(query_strings, "sp") + 1]);
        tcp::resolver resolver(io_context);
        tcp::resolver::iterator it = resolver.resolve(query);
        sock_endpoint = it->endpoint();
        set_sock = 1;
    }
